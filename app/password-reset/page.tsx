...
function PasswordResetContent() {
  ...
  useEffect(() => {
    ...
    const deepLink = `${DEEP_LINK_SCHEME}://password-reset?token=${encodeURIComponent(token)}`;
  ...
  if (!token) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F9FAFB] px-4">
        <p className="text-[#4B5563] text-center">Invalid reset link. Please use the link from your email.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#F9FAFB] px-4">
      <div className="max-w-md w-full bg-white rounded-xl shadow-sm border border-[#E5E7EB] p-8 text-center">
        <h1 className="text-xl font-semibold text-[#10121C] mb-2">Reset your password</h1>
